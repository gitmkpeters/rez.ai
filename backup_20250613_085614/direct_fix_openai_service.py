...
def fix_openai_service():
    ...
    # Find a good insertion point - before the last method in the class
    # This is more reliable than appending to the end of the file
    lines = content.split('\n')
    
    # Find all method definitions in the class
    method_lines = []
    for i, line in enumerate(lines):
        if line.strip().startswith('def ') and i > 0 and 'class' in lines[i-1]:
            method_lines.append(i)
        elif line.strip().startswith('def ') and i > 0 and lines[i-1].strip() == '':
            method_lines.append(i)
    ...
    if not method_lines:
        ...
        insertion_point = method_lines[-1]
